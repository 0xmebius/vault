...
import "src/integrations/JLPVault.sol";
...
contract TestJLPVault is DSTest {
    ...
    function setUp() public {
        ...
        _path[0] = Router.Node(JOEWAVAX, 1, JOE, address(WAVAX), 0, 0, 0);
        _path[1] = Router.Node(joePair, 2, address(WAVAX), joePair, 0, 0, 0);
        vault.setRoute(JOE, joePair, _path);
        ...
        vm.stopPrank();
        vm.startPrank(JLPHolder);
        JLP.transfer(address(this), JLP.balanceOf(JLPHolder));
        vm.stopPrank();
        

        // vault.pushRewardToken(address(QI));
        vault.pushRewardToken(JOE);

        JLP.approve(address(vault), MAX_INT);
        underlyingBalance=JLP.balanceOf(address(this));
    ...
    function testVanillaDepositNredeem(uint96 amt) public {
        ...
        uint preBalanceToken = JLP.balanceOf(address(this));
        vault.redeem(preBalanceVault);
        uint postBalanceVault = vault.balanceOf(address(this));
        uint postBalanceToken = JLP.balanceOf(address(this));
    ...
    function testVanillaDepositNCompoundredeem(uint96 amt) public returns (uint) {
        ...
        assertTrue(amt < JLP.balanceOf(address(this)));
        return amt;
    }
}
